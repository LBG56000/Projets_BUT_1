...
# Fonction brute_force testant toutes les possibilités lignes par lignes et colonnes par colonnes avec la position d'une reine imposée

def brute_force(echiquier, rangee, grille, position_imposee):
    """
        echiquier (liste): la configuration actuelle de l'échiquier.
        rangee (int): la rangée actuelle où une reine doit être placée.
        grille (int): la taille de la grille d'échecs.
        position_imposee (tuple): la position de la première reine qui doit être respectée.
    """
    global nb
...
def premiere_reine(message, grille):
    """
        message (str): un message indiquant quel est le but de cette position de reine.
        grille (int): la taille de la grille d'échecs.
...
def conflit_reines(echiquier, ligne, colonne, grille):
    """
        echiquier (liste): la configuration actuelle de l'échiquier.
        ligne (int): la ligne de la case à vérifier.
...
def afficher(echiquier):
    """
        echiquier (liste): la configuration actuelle de l'échiquier à afficher.
    """
...
def main():
    """
    La fonction principale du programme qui gère le processus global de résolution du problème des N-Reines.
    """
